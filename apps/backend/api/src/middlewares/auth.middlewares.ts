...
import ResetToken, { IResetToken } from '../models/ResetToken/reset-token';
import accessRIGHTS from '../models/User/type/role-types';
...
export function loginMiddleware(req: Request, res: Response) {
...
export async function SUDOMiddleware(
  req: Request,
  res: Response,
  next: NextFunction,
) {
...
export async function resetPassRequestMiddleWare(
  req: Request,
  res: Response,
  next: NextFunction,
) {
...
export async function resetPasswordMiddleware(
  ...
  let verifiedResetToken = (await ResetToken.findOne({
  ...
  req.body.user = verifiedResetToken.user;
